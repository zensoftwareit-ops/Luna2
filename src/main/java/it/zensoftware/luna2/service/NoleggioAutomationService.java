...
public class NoleggioAutomationService {
    ...
    private final NoleggioLeadDAO noleggioLeadDAO;
    ...
    public NoleggioAutomationService() {
        ...
        this.noleggioLeadDAO = new NoleggioLeadDAO();
    ...
    public void verificaFollowupPreventivi() {
        try {
            ...
            for (NoleggioPreventivo preventivo : requireFollowup) {
                try {
                    Lead lead = loadLeadByNoleggioId(preventivo.getNoleggioLeadId());
                    Long destinatarioId = lead != null && lead.getCliente() != null ? lead.getCliente().getId() : null;
                    String destinatarioLabel = buildLeadLabel(lead, preventivo.getNoleggioLeadId());
    ...
    public void verificaDocumentiScaduti() {
        try {
            ...
            for (NoleggioDocumento documento : expiring) {
                try {
                    Lead lead = loadLeadByNoleggioId(documento.getNoleggioLeadId());
                    Long destinatarioId = lead != null && lead.getCliente() != null ? lead.getCliente().getId() : null;
                    String destinatarioLabel = buildLeadLabel(lead, documento.getNoleggioLeadId());
                    // Notify lead's assigned user
                    if (destinatarioId != null) {
                        notificationService.sendNotificationToUser(
                            String.valueOf(destinatarioId),
                            "Documento in Scadenza",
                            documento.getTipoDocumento() + " per " + destinatarioLabel + 
                            " scade il " + documento.getDataScadenzaDocumento(),
    ...
    public void verificaValutazioniBloccate() {
        try {
            ...
            for (NoleggioValutazione valutazione : stalled) {
                try {
                    Lead lead = loadLeadByNoleggioId(valutazione.getLead().getId());
                    Long destinatarioId = lead != null && lead.getCliente() != null ? lead.getCliente().getId() : null;
                    String destinatarioLabel = buildLeadLabel(lead, valutazione.getLead().getId());
                    // Alert assigned manager/user
                    if (destinatarioId != null) {
                        notificationService.sendNotificationToUser(
                            String.valueOf(destinatarioId),
                            "⚠️ Valutazione Bloccata",
                            "La valutazione per " + destinatarioLabel + 
    ...
    public void verificaCareCall() {
        try {
            ...
            for (NoleggioOrdine ordine : requiring) {
                try {
                    Lead lead = loadLeadByNoleggioId(ordine.getLead().getId());
                    Long destinatarioId = lead != null && lead.getCliente() != null ? lead.getCliente().getId() : null;
    ...
    private Lead loadLeadByNoleggioId(Long noleggioLeadId) {
        if (noleggioLeadId == null) {
            return null;
        }
        NoleggioLead noleggioLead = noleggioLeadDAO.findById(noleggioLeadId);
        if (noleggioLead == null || noleggioLead.getLeadId() == null) {
            return null;
        }
        return leadDAO.findById(noleggioLead.getLeadId());
    }

    private String buildLeadLabel(Lead lead, Long noleggioLeadId) {
        if (lead == null) {
            return noleggioLeadId != null ? "NL-" + noleggioLeadId : "cliente";
        }
        if (lead.getAzienda() != null && !lead.getAzienda().isEmpty()) {
            return lead.getAzienda();
        }
        String nome = lead.getNomeContatto() != null ? lead.getNomeContatto() : "";
        String cognome = lead.getCognomeContatto() != null ? lead.getCognomeContatto() : "";
        String fullName = (nome + " " + cognome).trim();
        return !fullName.isEmpty() ? fullName : "NL-" + (noleggioLeadId != null ? noleggioLeadId : "cliente");
    }
}
