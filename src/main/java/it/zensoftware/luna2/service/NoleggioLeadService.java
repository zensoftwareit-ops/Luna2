...
public class NoleggioLeadService {
    ...
    public NoleggioLead createLead(NoleggioLead lead, Long userId) {
        ...
        lead.setUtenteCreazioneId(userId);
        
        // Generate unique code if not set
        if (lead.getCodiceNoleggio() == null || lead.getCodiceNoleggio().isEmpty()) {
            lead.setCodiceNoleggio(generateCodiceNoleggio());
        }
        
        leadDAO.save(lead);
        logger.info("Created noleggio lead: " + lead.getCodiceNoleggio());
        
        // Send notification to assigned user
        if (lead.getUtenteAssegnatoId() != null) {
            notificationService.sendNotificationToUser(
                String.valueOf(lead.getUtenteAssegnatoId()),
                "Nuovo Lead Noleggio",
                "Assegnato lead: " + lead.getNomeCliente(),
    ...
    public NoleggioPreventivo createPreventivo(Long leadId, NoleggioPreventivo preventivo, Long userId) {
        ...
        int nextVersion = (latest != null) ? latest.getNumeroVersione() + 1 : 1;
        
        preventivo.setLead(lead);
        preventivo.setNumeroVersione(nextVersion);
        preventivo.setDataCreazione(new Date());
        preventivo.setUtenteCreazioneId(userId);
        preventivo.setNumeroPreventivo(generaNumeroPreventivo(lead.getCodiceNoleggio(), nextVersion));
    ...
    public NoleggioPreventivo sendPreventivo(Long preventivoId, Long userId) {
        ...
        List<NoleggioPreventivo> previous = preventivoDAO.findByLead(preventivo.getLead().getId());
    ...
    public NoleggioLead acceptPreventivo(Long preventivoId, Long userId) {
        ...
        preventivo.setDataAccettazione(new Date());
        preventivoDAO.update(preventivo);
        
        // Advance lead to ISTRUTTORIA phase
        NoleggioLead lead = preventivo.getLead();
        lead.avanzaFase();
        leadDAO.update(lead);
        ...
        if (lead.getUtenteAssegnatoId() != null) {
            notificationService.sendNotificationToUser(
                ...
                "Lead " + lead.getCodiceNoleggio() + " avanzato a ISTRUTTORIA",
                NotificationEvent.Priority.HIGH
            );
        }
        
        logger.info("Accepted preventivo, lead advanced to ISTRUTTORIA: " + lead.getCodiceNoleggio());
        return lead;
    }
    ...
    public NoleggioDocumento createDocumento(Long leadId, NoleggioDocumento documento, Long userId) {
        ...
        documento.setUtenteCreazioneId(userId);
    ...
    public NoleggioDocumento requestDocumento(Long documentoId, Long userId) {
        ...
        documento.setStatus(NoleggioDocumento.Status.RICHIESTO);
        documento.setDataRichiesta(new Date());
    ...
    public NoleggioDocumento uploadDocumento(Long documentoId, String fileName, String filePath, Long fileSize, String mimeType) {
        ...
        documento.setStatus(NoleggioDocumento.Status.CARICATO);
    ...
    public NoleggioDocumento validateDocumento(Long documentoId, Long userId) {
        ...
        Long leadId = documento.getLead().getId();
    ...
    public NoleggioValutazione createValutazione(Long leadId, NoleggioValutazione valutazione, Long userId) {
        ...
        valutazione.setDataCreazione(new Date());
        valutazione.setUtenteCreazioneId(userId);
        ...
        logger.info("Created valutazione for lead: " + lead.getCodiceNoleggio());
    ...
    public NoleggioLead finalizeValutazione(Long valutazioneId, boolean approved, Long userId) {
        ...
        if (approved) {
            valutazione.setStatus(NoleggioValutazione.Status.VALUTAZIONE_POSITIVA);
            valutazione.setDataValutazioneFinale(new Date());
            ...
            if (lead.getUtenteAssegnatoId() != null) {
                notificationService.sendNotificationToUser(
                    ...
                    "Lead " + lead.getCodiceNoleggio() + " pronto per ordine",
                    NotificationEvent.Priority.HIGH
                );
            }
            
            logger.info("Valutazione approved, lead advanced to ORDINE: " + lead.getCodiceNoleggio());
            return lead;
        } else {
            valutazione.setStatus(NoleggioValutazione.Status.VALUTAZIONE_NEGATIVA);
            valutazione.setDataValutazioneFinale(new Date());
            valutazioneDAO.update(valutazione);
            
            logger.info("Valutazione rejected for lead: " + valutazione.getLead().getCodiceNoleggio());
    ...
    public NoleggioOrdine createOrdine(Long leadId, NoleggioOrdine ordine, Long userId) {
        ...
        ordine.setUtenteCreazioneId(userId);
        ordine.setNumeroOrdine(generaNumeroOrdine(lead.getCodiceNoleggio()));
    ...
    public NoleggioOrdine updateOrdineStatus(Long ordineId, NoleggioOrdine.Status newStatus, Long userId) {
        ...
        if (newStatus == NoleggioOrdine.Status.CONSEGNATO) {
            ...
            logger.info("Order delivered, lead advanced to POST_VENDITA: " + lead.getCodiceNoleggio());
        }
    ...
    public NoleggioOrdine registerCareCall(Long ordineId, String note, Long userId) {
        ...
        ordine.setNote(ordine.getNote() + "\n[Care Call " + new Date() + "] " + note);
        ordineDAO.update(ordine);
    ...
    private String generateCodiceNoleggio() {
        return "NLG" + System.currentTimeMillis();
    }
