...
final class CommunicationsController extends BaseController
{
    ...
    private function documentPdf(int $documentId):array
    {
        ...
        $partyTable=$document['counterparty_type']==='SUPPLIER'?'suppliers':'customers';$counterparty=$this->one("SELECT * FROM {$partyTable} WHERE id=? AND organization_id=?",[$document['counterparty_id'],Auth::organizationId()])?:[];
