...
6. [Fase 4: Installazione Luna2](#fase-4-installazione-luna2)
...
### 4.2 Configurazione Multi-Tenant

**⚠️ IMPORTANTE:** Per il setup multi-tenant con Server Manager, **NON serve** configurare un file `.env` globale!

**Perché?**
- ✅ Ogni istanza cliente avrà il proprio database MySQL con credenziali **uniche**
- ✅ Le credenziali vengono **generate automaticamente** dal Server Manager quando crei un'istanza
- ✅ Ogni istanza è **completamente isolata** dalle altre
- ✅ Il Server Manager genera un `docker-compose-<dominio>.yml` specifico per ogni cliente

**Esempio configurazione automatica per un'istanza:**
```yaml
# Generato automaticamente per cliente1.example.com
services:
  mysql:
    environment:
      MYSQL_ROOT_PASSWORD: AutoGeneratedPassword_XYZ123
      MYSQL_DATABASE: luna2_cliente1
      MYSQL_USER: luna2_cliente1
      MYSQL_PASSWORD: AutoGeneratedPassword_ABC456
```

**💡 TIP:** Ogni volta che crei una nuova istanza via pannello Server Manager, verrà generato un set completo di credenziali uniche per quella specifica istanza.

**📝 NOTA:** Se vuoi deployare una **singola istanza** (non multi-tenant), allora sì, dovrai configurare un file `.env`. Ma questa guida è per setup multi-tenant, quindi puoi **saltare questo step**.

### 4.3 Prima Build (Compilazione WAR)
...
✅ Se vedi il file WAR → Compilazione OK

### 4.4 Configurazione Docker Compose
...
**Dovresti vedere:**
- `docker-compose-main.yml` - Setup base singola istanza
- `docker-compose-preprod.yml` - Setup pre-produzione
- `docker-compose-demo.yml` - Setup demo

**Per produzione multi-tenant, useremo setup personalizzato.**
