...
$assert(str_contains($pleskClient, "'-seo-redirect', 'false'") && str_contains($pleskClient, "'-status', 'enabled'"), 'L’alias personalizzato non deve reindirizzare al dominio tecnico.');
...
$assert(
    str_contains($documentController, ':search_number')
    && str_contains($documentController, ':search_counterparty')
    && str_contains($documentController, ':search_subject')
...
try {
    ...
    $renderedAccounting .= $renderAccountingView('trial-balance', ['sections' => ['ASSET'=>[],'LIABILITY'=>[],'EQUITY'=>[],'REVENUE'=>[],'EXPENSE'=>[]], 'totals' => ['debit'=>0,'credit'=>0,'assets'=>0,'liabilities'=>0,'expenses'=>0,'revenues'=>0,'profit_loss'=>0,'balance_assets'=>0,'balance_liabilities'=>0], 'opening' => ['posted'=>false,'carried'=>false,'adjustment'=>0], 'from' => '2026-01-01', 'to' => '2026-12-31', 'search' => '', 'accountType' => '']);
