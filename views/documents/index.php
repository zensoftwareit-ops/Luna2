...
<section class="filter-toolbar-card">
    ...
    <form method="get" class="document-filter-grid">
        ...
        <label class="field"><span>Stato</span><select name="status"><option value="">Tutti</option><?php foreach (['DRAFT','SENT','ACCEPTED','REJECTED','CONFIRMED','IN_PROGRESS','FULFILLED','ISSUED','RECEIVED','PARTIALLY_PAID','PAID','OVERDUE','CANCELLED','HISTORICAL'] as $value): ?><option value="<?= $value ?>" <?= $status === $value ? 'selected' : '' ?>><?= View::e(View::label($value)) ?></option><?php endforeach; ?></select></label>
